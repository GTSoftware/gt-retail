...
public class LegalControllerImpl implements LegalController {
  ...
  public List<LegalGenerosDto> getGenders(Long legalStatusId) {
    final GenerosSearchFilter searchFilter =
        GenerosSearchFilter.builder().idTipoPersoneria(legalStatusId).build();
  ...
  public List<IdentificationType> getIdentificationTypes(Long legalStatusId) {
    final LegalTiposDocumentoSearchFilter searchFilter =
        LegalTiposDocumentoSearchFilter.builder().idTipoPersoneria(legalStatusId).build();

    return transformIdentificationTypes(tiposDocumentoService.findAllBySearchFilter(searchFilter));
  }

  private List<IdentificationType> transformIdentificationTypes(
      List<LegalTiposDocumentoDto> tiposDocumentoDtos) {
