...
public class PaymentPendingSalesControllerImpl implements PaymentPendingSalesController {
  ...
  public PaginatedResponse<PaymentPendingSale> findBySearchFilter(
      @Valid PaginatedSearchRequest<ComprobantesSearchFilter> request) {
    ...
    return PaginatedResponseBuilder.build(comprobantesService, request, pendingSaleTransformer);
  }
  ...
  private List<PaymentMethod> getNoExtraCostPaymentMethods(List<SaleToPayDto> salesToPay) {
    final Optional<SaleToPayDto> undefinedPayment =
        salesToPay.stream().filter(sale -> isNull(sale.getPayment())).findAny();
  ...
  private List<SaleToPay> transformSalesToPay(List<SaleToPayDto> salesToPayDto) {
    ...
    salesToPayDto.forEach(saleToPayDto -> salesToPay.add(buildSaleToPay(saleToPayDto)));
