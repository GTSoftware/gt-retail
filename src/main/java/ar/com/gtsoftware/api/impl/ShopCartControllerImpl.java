...
class ShopCartControllerImpl implements ShopCartController {
  ...
  public List<Customer> searchCustomers(String query) {
    ...
    return customerTransformer.transformCustomers(
        personasService.findBySearchFilter(filter, 0, 15));
  }

  @Override
  public List<NegocioTiposComprobanteDto> getSaleTypes() {
    NegocioTiposComprobanteSearchFilter tipoCompSf =
        NegocioTiposComprobanteSearchFilter.builder().activo(true).build();
    List<NegocioTiposComprobanteDto> tiposComprobanteDtos =
        negocioTiposComprobanteService.findAllBySearchFilter(tipoCompSf);
  ...
  public List<NegocioCondicionesOperacionesDto> getSaleConditions() {
    final Long defaultSaleConditionId =
        parametrosService.getLongParam(Parametros.ID_CONDICION_VENTA_DEFECTO);
    List<NegocioCondicionesOperacionesDto> condiciones =
        negocioCondicionesOperacionesService.findAll();
  ...
  public List<NegocioFormasPagoDto> getPaymentMethods() {
    final FormasPagoSearchFilter formasPagoSearchFilter =
        FormasPagoSearchFilter.builder().disponibleVenta(true).build();
    final Long defaultPaymentMethodId =
        parametrosService.getLongParam(Parametros.ID_FORMA_PAGO_DEFECTO);
    List<NegocioFormasPagoDto> paymentMethods =
        negocioFormasPagoService.findAllBySearchFilter(formasPagoSearchFilter);
  ...
  public List<NegocioPlanesPagoDto> getPaymentPlans(Long paymentPlanId) {
    final PlanesPagoSearchFilter sf =
        PlanesPagoSearchFilter.builder().activo(true).idFormaPago(paymentPlanId).build();
  ...
  public CreatedSaleResponse saveSale(SaleRequest saleRequest) {
    ...
    ComprobantesDto comprobante =
        ComprobantesDto.builder()
            .idCondicionComprobante(
                NegocioCondicionesOperacionesDto.builder()
                    .id(saleRequest.getSaleConditionId())
                    .build())
            ...
            .tipoComprobante(
                NegocioTiposComprobanteDto.builder().id(saleRequest.getSaleTypeId()).build())
  ...
  private List<ComprobantesPagosDto> transformPayments(List<SalePayment> payments) {
    ...
    for (SalePayment payment : payments) {
      ...
      if (payment.getPaymentPlanId() != null) {
        ...
        pago.setIdDetallePlan(
            NegocioPlanesPagoDetalleDto.builder().id(payment.getPaymentPlanDetailId()).build());
      }
      pagos.add(pago);
    }
  ...
  private List<ComprobantesLineasDto> transformSaleItems(List<SaleItem> products) {
    ...
    for (SaleItem item : products) {
      ...
      if (item.getId() != null) {
        producto =
            ProductosDto.builder().id(item.getId()).codigoPropio(item.getCodigoPropio()).build();
      } else {
      ...
      discountItem =
          DiscountItem.builder()
              ...
              .descripcion(
                  StringUtils.left(
                      "["
                          + linea.getIdProducto().getId()
                          + "] "
                          + promotionCartItem.getNombreOferta(),
                      90))
              .codigoPropio("DESC.")
