...
public class RemitoDtoTransformer {
    ...
    public RemitoDto transformAddDeliveryNote(AddDeliveryNoteRequest request) {
        final RemitoDto remitoDto = RemitoDto.builder()
                .idUsuario(UsuariosDto.builder().id(securityUtils.getUserDetails().getId()).build())
                .observaciones(request.getObservations())
                .remitoTipoMovimiento(RemitoTipoMovimientoDto.builder().id(request.getDeliveryTypeId()).build())
                .build();

        if (request.getExternalDestinationId() != null) {
            remitoDto.setIdDestinoPrevistoExterno(PersonasDto.builder().id(request.getExternalDestinationId()).build());
        ...
        if (request.getInternalDestinationId() != null) {
            remitoDto.setIdDestinoPrevistoInterno(DepositosDto.builder().id(request.getInternalDestinationId()).build());
            remitoDto.setIsDestinoInterno(true);
        }
        if (request.getInternalOriginId() != null) {
            remitoDto.setIdOrigenInterno(DepositosDto.builder().id(request.getInternalOriginId()).build());
            remitoDto.setIsOrigenInterno(true);
        }

        remitoDto.setDetalleList(remitoDetalleTransformer.transformDeliveryItems(request.getDeliveryNoteItems()));
