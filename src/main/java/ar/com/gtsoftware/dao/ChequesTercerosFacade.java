...
@Repository
public class ChequesTercerosFacade
    extends AbstractFacade<ChequesTerceros, ChequesTercerosSearchFilter> {

  private final EntityManager em;

  public ChequesTercerosFacade(EntityManager em) {
  ...
  public Predicate createWhereFromSearchFilter(
      ChequesTercerosSearchFilter sf, CriteriaBuilder cb, Root<ChequesTerceros> root) {
    ...
    if (sf.hasFilterFechaOrigen()) {
      Predicate p1 =
          cb.between(
              root.get(ChequesTerceros_.fechaOrigen),
              sf.getFechaOrigenDesde(),
