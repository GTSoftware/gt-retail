...
@Repository
public class ComprobantesPagosFacade
    extends AbstractFacade<ComprobantesPagos, ComprobantesPagosSearchFilter> {
  ...
  public Predicate createWhereFromSearchFilter(
      ComprobantesPagosSearchFilter sf, CriteriaBuilder cb, Root<ComprobantesPagos> root) {
    Predicate p = null;
    if (sf.getIdComprobante() != null) {
      Predicate p1 =
          cb.equal(
              root.get(ComprobantesPagos_.idComprobante).get(Comprobantes_.id),
              sf.getIdComprobante());
    ...
    if (sf.getConSaldo() != null) {
      Predicate p1 =
          cb.equal(
              root.get(ComprobantesPagos_.montoPago), root.get(ComprobantesPagos_.montoPagado));
      if (sf.getConSaldo()) {
        p1 = p1.not();
      }
      p = appendAndPredicate(cb, p, p1);
