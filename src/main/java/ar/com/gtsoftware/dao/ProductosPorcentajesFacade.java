...
@Repository
public class ProductosPorcentajesFacade
    extends AbstractFacade<ProductosPorcentajes, ProductosPorcentajesSearchFilter> {
  ...
  public Predicate createWhereFromSearchFilter(
      ProductosPorcentajesSearchFilter psf, CriteriaBuilder cb, Root<ProductosPorcentajes> root) {

    Predicate p = null;
    if (psf.getIdProducto() != null) {
      Predicate p1 =
          cb.equal(
              root.get(ProductosPorcentajes_.idProducto).get(Productos_.id), psf.getIdProducto());
      p = appendAndPredicate(cb, p, p1);
    }
    if (psf.getIdTipoPorcentaje() != null) {
      Predicate p1 =
          cb.equal(
              root.get(ProductosPorcentajes_.idTipoPorcentaje).get(ProductosTiposPorcentajes_.id),
              psf.getIdTipoPorcentaje());
      p = appendAndPredicate(cb, p, p1);
    }
    return p;
  }
