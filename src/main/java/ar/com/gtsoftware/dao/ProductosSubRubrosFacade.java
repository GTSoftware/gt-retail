...
@Repository
public class ProductosSubRubrosFacade
    extends AbstractFacade<ProductosSubRubros, SubRubroSearchFilter> {
  ...
  public Predicate createWhereFromSearchFilter(
      SubRubroSearchFilter psf, CriteriaBuilder cb, Root<ProductosSubRubros> root) {
    ...
    if (psf.getIdProductosRubros() != null) {
      Predicate p1 =
          cb.equal(
              root.get(ProductosSubRubros_.idRubro).get(ProductosRubros_.id),
              psf.getIdProductosRubros());
