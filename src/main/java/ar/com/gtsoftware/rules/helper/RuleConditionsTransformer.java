...
public class RuleConditionsTransformer {
    ...
    public static String transformConditions(List<Condicion> condiciones) throws CondicionIlegalException {
    ...
    private static void validarCondicion(Condicion condition) throws CondicionIlegalException {
        if (condition.getOperacion() == null || condition.getCampo() == null || isEmpty(condition.getValor())) {
        ...
        if (!condition.getOperacion().soportaTipo(condition.getCampo().getClase())) {
            throw new CondicionIlegalException("El campo:" + condition.getCampo() + " no soporta la operación: " + condition.getOperacion());
        }
