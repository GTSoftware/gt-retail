...
public class FacturacionVentasServiceImpl implements FacturacionVentasService {
  ...
  @Override
  @Transactional
  public void registrarFacturaVenta(
    ...
    FiscalPeriodosFiscalesSearchFilter psf =
        FiscalPeriodosFiscalesSearchFilter.builder().cerrado(false).build();
    ...
    FiscalTiposComprobanteSearchFilter ftcsf =
        FiscalTiposComprobanteSearchFilter.builder()
            ...
            .idTipoComprobante(venta.getTipoComprobante().getId())
            .build();

    FiscalTiposComprobante tipoCompFiscal =
        fiscalTiposComprobanteFacade.findFirstBySearchFilter(ftcsf);
    if (tipoCompFiscal == null) {
      throw new ServiceException("Este comprobante no puede ser fiscalizado!");
    }
    FiscalPuntosVenta fiscalPuntosVenta =
        puntosVentaMapper.dtoToEntity(puntoVentaComprobanteDto, new CycleAvoidingMappingContext());
  ...
  private void calcularIVA(
      List<ComprobantesLineas> lineasVenta, FiscalLibroIvaVentas registro, BigDecimal signo) {
    ...
    for (ComprobantesLineas vl : lineasVenta) {
      ...
      TotalesAlicuotas subTotal =
          TotalesAlicuotas.builder()
      ...
      if (alicuota.isGravarIva()) {
        ...
        BigDecimal coeficienteIVA =
            BigDecimal.ONE.add(alicuota.getValorAlicuota().divide(new BigDecimal(100)));
    ...
    BigDecimal totalIVA = BigDecimal.ZERO,
        totalNetoGravado = BigDecimal.ZERO,
        totalNetoNoGravado = BigDecimal.ZERO;
    ...
    for (TotalesAlicuotas total : totales) {
      ...
      registroLinea.setIdAlicuotaIva(
          alicuotasIvaMapper.dtoToEntity(total.getAlicuota(), new CycleAvoidingMappingContext()));
  ...
  private void generarFacturaElectronica(
    ...
    int ultimoNro =
        afipService.obtenerUltimoComprobanteAutorizado(
            puntoVentaComprobante, registro.getCodigoTipoComprobante());
  ...
  public long obtenerProximoNumeroFactura(String letra, int puntoVenta) {
    FiscalLibroIvaVentas ultimaFactura =
        ivaVentasFacade.findUltimaFactura(letra, formatPuntoVenta(puntoVenta));
    long nro = 1;
