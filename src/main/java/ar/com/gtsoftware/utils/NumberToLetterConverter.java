...
/**
 * Esta clase provee la funcionalidad de convertir un numero representado en digitos a una
 * representacion en letras. Mejorado para leer centavos
 *
 * @version 1.0
 */
public abstract class NumberToLetterConverter {
   ...
   *
   * <p>Creation date 3/05/2006 - 05:37:47 PM
   *
   ...
   * numero.
   *
   * <p>Creation date 3/05/2006 - 05:33:40 PM
   *
  ...
  private static String convertNumber(String number) {
    if (number.length() > 3) {
      throw new IllegalArgumentException("La longitud máxima debe ser 3 dígitos");
    }
    ...
    int k =
        Integer.parseInt(
            String.valueOf(getDigitAt(number, 1)).concat(String.valueOf(getDigitAt(number, 0))));

    if (k <= 20) {
      output.append(UNIDADES[k]);
    } else if (k > 30 && getDigitAt(number, 0) != 0) {
      output
          .append(DECENAS[getDigitAt(number, 1) - 2])
          .append(Y)
