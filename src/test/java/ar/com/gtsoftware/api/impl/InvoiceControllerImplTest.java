...
class InvoiceControllerImplTest {
    ...
    @Mock
    private FiscalPuntosVentaService puntosVentaServiceMock;
    @Mock
    private ComprobantesService comprobantesServiceMock;
    @Mock
    private FacturacionVentasService facturacionServiceMock;


    @Captor
    private ArgumentCaptor<FiscalPuntosVentaSearchFilter> captor;

    @BeforeEach
    void setUp() {
        initMocks(this);
        controller = new InvoiceControllerImpl(
                securityUtilsMock,
    ...
    public void shouldGetPointsOfSale() {
        ...
        assertThat(filter.getTiposPuntoVenta(), containsInAnyOrder(
                TiposPuntosVenta.ELECTRONICO,
                TiposPuntosVenta.MANUAL)
        );
    ...
    public void shouldInvoice() throws ServiceException {
        final FiscalPuntosVentaDto puntoVentaDummy = FiscalPuntosVentaDto.builder().nroPuntoVenta(1).build();
        when(comprobantesServiceMock.find(1L))
                .thenReturn(ComprobantesDto.builder().id(1L).build())
                .thenReturn(ComprobantesDto.builder().id(1L).idRegistro(FiscalLibroIvaVentasDto.builder()
                        .letraFactura("B")
        ...
        final InvoiceRequest invoiceRequest = InvoiceRequest.builder()
                .pointOfSale(1)
                .saleId(1L)
                .build();
        ...
        verify(facturacionServiceMock).registrarFacturaVenta(eq(1L), eq(puntoVentaDummy), eq(0L), isNull());
    ...
    public void shouldThrowSaleNotFoundWhenInvoiceWithInvalidSaleId() {
        ...
        Assertions.assertThrows(SaleNotFoundException.class, () -> controller.invoiceSale(invoiceRequest));
    ...
    public void shouldThrowPointOfSaleNotFoundWhenInvoiceWithInvalidPointOfSale() {
        ...
        Assertions.assertThrows(PointOfSaleNotFoundException.class, () -> controller.invoiceSale(invoiceRequest));
