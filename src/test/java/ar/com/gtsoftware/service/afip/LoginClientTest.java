...
class LoginClientTest {
  ...
  @Autowired private LoginClient client;
  ...
  void shouldLogin() {
    ...
    when(client.getLoginTicketRequestBuilder().buildLoginTicketRequest("test"))
        .thenReturn(loginTicketRequest);
    when(client.getLoginRequestSigner().getSignedTicketRequest(loginTicketRequest))
