...
class PaymentsServiceImplTest {
    ...
    void setUp() {
        initMocks(this);
        paymentsService = new PaymentsServiceImpl(mockRecibosFacade,
    ...
    void shouldPrepareToPayWhenOneCashPaymentIsPresent() {
        ...
        when(mockPersonasMapper.entityToDto(eq(comp.getIdPersona()), any())).thenReturn(PersonasDto.builder().id(2L).build());
        when(mockComprobantesPagosMapper.entityToDto(eq(pago), any())).thenReturn(ComprobantesPagosDto.builder().build());
        when(mockComprobantesMapper.entityToDto(eq(comp), any())).thenReturn(ComprobantesDto.builder().build());
        final PreparedPaymentDto preparedPaymentDto = paymentsService.prepareToPay(Collections.singletonList(1L));
    ...
    void shouldPrepareToPayWhenRequiredValuePaymentIsPresent() {
        ...
        when(mockComprobantesFacade.find(1L)).thenReturn(comp);
        when(mockPersonasMapper.entityToDto(eq(comp.getIdPersona()), any())).thenReturn(PersonasDto.builder().id(2L).build());
        when(mockComprobantesPagosMapper.entityToDto(eq(comp.getPagosList().get(0)), any())).thenReturn(ComprobantesPagosDto.builder().build());
        when(mockComprobantesMapper.entityToDto(eq(comp), any())).thenReturn(ComprobantesDto.builder().build());
        final PreparedPaymentDto preparedPaymentDto = paymentsService.prepareToPay(Collections.singletonList(1L));
    ...
    void shouldNotValidatePrepareToPayWhenSalesAreForDifferentCustomers() {
        ...
        Assertions.assertThrows(RuntimeException.class,
                () -> paymentsService.prepareToPay(Arrays.asList(1L, 2L)));
