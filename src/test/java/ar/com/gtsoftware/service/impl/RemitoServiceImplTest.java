...
class RemitoServiceImplTest {
  ...
  void shouldGetProperPartialStockWhenInternalExternalDelivery() {
    ...
    when(remitoDetalleFacadeMock.findAllBySearchFilter(remitoDetalleSearchFilter))
        .thenReturn(buildDummyDetalles());
    final List<ProductoMovimiento> movimientosProducto =
        service.getMovimientosProducto(remitoDetalleSearchFilter);
  ...
  void shouldGetProperPartialStockWhenInternalDelivery() {
    final RemitoDetalleSearchFilter remitoDetalleSearchFilter =
        RemitoDetalleSearchFilter.builder().idDepositoMovimiento(1L).build();

    when(stockFacadeMock.getStockBySearchFilter(any())).thenReturn(BigDecimal.TEN);
    when(remitoDetalleFacadeMock.findAllBySearchFilter(remitoDetalleSearchFilter))
        .thenReturn(buildDummyDetallesInterno());
    final List<ProductoMovimiento> movimientosProducto =
        service.getMovimientosProducto(remitoDetalleSearchFilter);
