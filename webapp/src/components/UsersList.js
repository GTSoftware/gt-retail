...
export class UsersList extends Component {
    ...
    componentDidMount() {
        UsersService.retrieveUsers()
            .then(
                response => this.handleRetrieveUsers(response.data)
            );

        SucursalesService.retrieveActiveSucursales()
            .then(
                response => this.handleRetrieveSucursales(response.data)
            );
    }
    ...
    renderMainContent() {
        return (
            <div className="card card-w-title">
                ...
                <DataTable {...this.getUsersTableProps()}>
                    {this.renderColumns()}
                </DataTable>
                {this.renderDialog()}
            </div>
    ...
    getUsersTableProps() {
        let header = <div className="p-clearfix" style={{lineHeight: '1.87em'}}>Usuarios</div>;
        let footer = <div className="p-clearfix" style={{width: '100%'}}>
            <Button style={{float: 'left'}} label="Nuevo" icon="fa fa-fw fa-plus" onClick={this.addNewUser}/>
        </div>;
    ...
    getTableActions(rowData, column) {
        return (
            <div className="p-grid p-fluid">
                <Button type="button" icon="fa fa-fw fa-lock" className="p-button-warning" tooltip={'Resetear clave'}
                        onClick={() => (this.handleResetPassword(rowData))}/>
                <Button type="button" icon="fa fa-fw fa-edit" tooltip={'Editar usuario'}
                    ...
                    onHide={() => this.setState({displayDialog: false})}>
                {
                    ...
                    <div className="p-grid p-fluid">
                        ...
                        <div className="p-col-4" style={{padding: '.75em'}}><label
                            htmlFor="nombreUsuario">Nombre</label>
                        </div>
                        <div className="p-col-8" style={{padding: '.5em'}}>
