...
const BOOK_KINDS = [{label: 'Compras y Ventas', key: 'BOTH'}, {label: 'Compras', key: 'PURCHASES'}, {
    label: 'Ventas',
    key: 'SALES'
}];
const EXPORT_OPTIONS = [{label: 'Régimen informativo', key: 'REGINFO'}, {
...
export class FiscalBooks extends Component {
    ...
    componentDidMount() {
        ...
        if (fiscalPeriods.length === 0) {
            this.fiscalBookService.getFiscalPeriods((fiscalPeriods) => this.setState({
    ...
    downloadFiscalBook = () => {
        ...
        if (fileType.key === 'REGINFO') {
            this.fiscalBookService.getRegInfoFile(filter, this.handleSuccess, this.handleError);
        } else {
            this.fiscalBookService.getFiscalBookSpreadsheetFile(filter, this.handleSuccess, this.handleError);
        }
